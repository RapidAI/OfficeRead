...
func main() {
	...
	flag.Parse()
	if flag.NArg() == 0 {
		fmt.Fprintln(os.Stderr, "usage: compatcheck [-json report.json] [-csv report.csv] [-limit n] [-repeat n] file-or-directory [...]")
		os.Exit(2)
	}
	if *repeat <= 0 {
...
func checkOne(path string, repeat int) (result FileResult) {
	if repeat <= 1 {
		return checkOneRun(path)
	}
	result = checkOneRun(path)
	result.Runs = []int64{result.Millis}
	result.MinMillis = result.Millis
	result.MaxMillis = result.Millis
	for i := 1; i < repeat; i++ {
		run := checkOneRun(path)
		...
		if run.TextBytes != result.TextBytes || run.Images != result.Images {
			if result.Error == "" {
				result.Error = fmt.Sprintf("repeat output mismatch: first text=%d images=%d, run %d text=%d images=%d", result.TextBytes, result.Images, i+1, run.TextBytes, run.Images)
			}
			result.OK = false
		}
	}
...
func checkOneRun(path string) (result FileResult) {
	result.Path = path
...
func writeCSV(path string, files []FileResult) error {
	...
	w := csv.NewWriter(f)
	defer w.Flush()
	if err := w.Write([]string{"path", "ext", "ok", "empty", "textBytes", "images", "error", "panic", "millis", "minMillis", "maxMillis", "runs"}); err != nil {
		return err
	}
	for _, r := range files {
		runs := ""
...
func hasCrash(report Report) bool {
